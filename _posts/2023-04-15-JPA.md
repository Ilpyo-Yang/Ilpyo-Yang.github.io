...
## 스프링 데이터 JPA
**반환값이 없는 경우 JPA와의 차이**  
JPA에서는 조회에 따른 반환값이 없는 경우 ```NoResultException```이 발생합니다. 하지만, JPA와 달리 스프링 데이터 JPA에서는 반환타입에 따라 결과값이 상이하게 나옵니다. 예를 들어 ```List``` 타입에서는 조회결과 값이 없으면, 빈 컬렉션을 반환하고, 단 건 조회해서 반환값이 없으면, ```null```을 반환합니다. 
만약 단 건 조회인데, 결과값이 2개 이상일 경우에는 ```javax.persistence.NonUniqueResultException``` 예외 발생합니다. 
