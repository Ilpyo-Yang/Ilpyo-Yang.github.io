...
## 연관관계 매핑
JPA를 처음 배우는 사람들이 가장 헷갈리고 어렵게 느낄 수 있는 부분일 것 같다고 생각합니다. 저 역시 그랬는데 [김영한님 인프런 로드맵](https://www.inflearn.com/roadmaps/149)을 들으면서 학습 내용을 정리하며 이해하기 쉬워졌습니다.  
...
<span style="background-color:#fff5b1">연관관계의 주인은 외래키가 있는 테이블과 매핑되는 entity에 설정하는 것을 권장합니다. 그 반대쪽에는 ```MappedBy```로 관계를 매핑해주어야 합니다.</span>  

맨 처음 예시로 들었던 학생과 반에 대한 연관관계 entity를 코드로 작성했습니다. 아래 코드에서는 ```일대다 양방향 매핑```이 된 상태입니다.  
```java
// 반 entity
