...
**성능을 최적화하는 방법**  
JPA에서 연관관계를 사용하면 ```N+1``` 문제를 마주치게 되거나 연관관계 조회로 인한 무한루프에 빠질 수 있습니다. 다양한 성능 최적화 방법이 있고, 상황별로 각 프로그램에 적절한 대안을 찾아 사용해야 합니다.  

<span style="background-color:#FFE6E6; margin-right:5px">Fetch 조인과 DTO 처리</span>  
Entity가 아닌 ```DTO```로 처리하고 ```fetch 조인```으로 성능을 최적화합니다. 여기서 ```distinct```를 사용하는 경우에는 페이징이 불가하다는 점에 유의합니다.  
일반적으로는 아래 코드와 같은 방법으로 대부분의 JPA 성능을 최적화할 수 있습니다. 조회 대상을 기준으로 ```xToOne```에 해당되는 경우는 ```fetch 조인```으로 성능을 최적화하는데, N번위 쿼리가 더 발생하지 않기 때문에 성능에 문제가 발생하지 않습니다. 그리고 이외에 N의 문제가 발생할 수 있는 부분은 ```DTO``` 매핑을 해주는 과정에서 지연로딩으로 인한 조회가 가능하게 됩니다.  
```Java
// OrderController
...
```
여기서 더 성능을 최적화하기 위해서 ```@BatchSize```, ```hibernate.default_batch_fetch_size```를 설정하는 방법도 있습니다. <span style="background-color:#fff5b1; margin-right:5px">DB와 애플리케이션에서 견딜 수 있는 순간 부하 기준을 확인하고 설정할 필요가 있습니다.</span>  
더 적절한 대응이 필요한 경우에는 데이터베이스단에서 ```DTO```에 속하는 속성만을 조회해서 리턴하거나, ```Native query```나 ```JDBC Template```를 사용합니다. 쿼리 수가 줄어들지만 코드 복잡성이나 재활용성이 떨어지기 때문에 항상 best라고 할 수는 없습니다.  
```java
private List<OrderQueryDto> findOrders() {
  return em.createQuery(
          "select new my.study.jpa.repository.order.query.OrderQueryDto(o.id, m.name, o.orderDate, o.status, d.address)" +
          " from Order o" +
              " join o.member m" +
...
## 스프링 데이터 JPA
스프링 개발자라면 누구나 사용하고 있을 스프링 데이터 JPA입니다! 스프링 데이터 JPA를 사용하면 기존 JPA보다 구현이나 런타임에서 에러를 발견하기 유리한 기능들을 제공합니다. 
```
...
**반환값이 없는 경우 JPA와의 차이**  
JPA에서는 조회에 따른 반환값이 없는 경우 ```NoResultException```이 발생합니다. 하지만, JPA와 달리 스프링 데이터 JPA에서는 반환타입에 따라 결과값이 상이하게 나옵니다. <span style="background-color:#fff5b1; margin-right:5px">예를 들어 ```List``` 타입에서는 조회결과 값이 없으면, ```빈 컬렉션```을 반환하고, 단 건 조회해서 반환값이 없으면, ```null```을 반환합니다. 
만약 단 건 조회인데, 결과값이 2개 이상일 경우에는 ```javax.persistence.NonUniqueResultException``` 예외 발생합니다.</span>  

**Page와 Slice**  
//countQuery
//벌크성 수정쿼리 ```@Modifying(clearAutomatically = true)``` 영속성 컨텍스트 초기화
...
****
[Spring Data JPA - Reference Documentation](https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#upgrading)  
[실전! 스프링 부트와 JPA 활용2 - API 개발과 성능 최적화](https://www.inflearn.com/course/%EC%8A%A4%ED%94%84%EB%A7%81%EB%B6%80%ED%8A%B8-JPA-API%EA%B0%9C%EB%B0%9C-%EC%84%B1%EB%8A%A5%EC%B5%9C%EC%A0%81%ED%99%94)
