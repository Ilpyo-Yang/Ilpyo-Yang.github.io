...
처리율 제한장치 미들웨어로 만들어 많은 요청이 발생한 경우 429 Too many request를 반환합니다. 마이크로서비스에서는 보통 API 게이트웨이라는 컴포넌트에 설계되는데, SSL 종단, 사용자 인증, IP 허용목록 관리 등을 지원하는 완전 위탁관리형 서비스입니다.  
...
+ 서버가서버 가 추가 또는 삭제될 때 재배치되는 키의 수가 최소화됩니다.
