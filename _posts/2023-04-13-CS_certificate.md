...
## Contents
+ [1장. 요구사항 확인](/cs/2023/04/13/CS_certificate.html#1장-요구사항-확인)
+ [2장. 데이터 입출력 구현](/cs/2023/04/13/CS_certificate.html#2장-데이터-입출력-구현)
+ [3장. 통합구현](/cs/2023/04/13/CS_certificate.html#3장-통합-구현)
+ [4장. 서버 프로그램 구현](/cs/2023/04/13/CS_certificate.html#4장-서버-프로그램-구현)
+ [5장. 인터페이스 구현](/cs/2023/04/13/CS_certificate.html#5장-인터페이스-구현)
+ [6장. 화면설계](/cs/2023/04/13/CS_certificate.html#6장-화면설계)
+ [7장. 애플리케이션 테스트 관리](/cs/2023/04/13/CS_certificate.html#7장-애플리케이션-테스트-관리)
+ [8장. SQL 응용](/cs/2023/04/13/CS_certificate.html#8장-SQL-응용)
+ [9장. 소프트웨어 개발 보안 구축](/cs/2023/04/13/CS_certificate.html#9장-소프트웨어-개발-보안-구축)
+ [10장. 프로그래밍 언어 활용](/cs/2023/04/13/CS_certificate.html#10장-프로그래밍-언어-활용)
+ [11장. 응용 SW 기초 기술 활용](/cs/2023/04/13/CS_certificate.html#11장-응용-sw-기초-기술-활용)
+ [12장. 제품 소프트웨어 패키징](/cs/2023/04/13/CS_certificate.html#12장-제품-소프트웨어-패키징)
+ [13장. 기타 추가내용](/cs/2023/04/13/CS_certificate.html#13장-기타-추가내용)
...
고객의 요구사항을 정리해 놓은 시나리고를 4개의 관점에서 바라보는 소프트웨어적인 접근방법
+ 유스케이스 뷰
...
| 데이터링크 계층 (Datalint Layer)  | 인접 시스템 간 데이터 전송, 전송오류 제어, 오류검출 및 재전송 기능 제공 | 이더넷        | 프레임  | 브리지, 스위치 |
...
+ ICMP(Internet Control Message Protocol) 🔴
  ...
  + 수신지 도달 불가 메시지는 수시낮 또는 서비스에 도달할 수 없는 호스트를 통지하는데 사용
...
|------------|----------|----------|
| ![class_diagram.png](/assets/gitbook/post_images/cs/class_diagram.png)           |![component_diagram.gif](/assets/gitbook/post_images/cs/component_diagram.gif)          |![uml_diagram.png](/assets/gitbook/post_images/cs/uml_diagram.png)|

| 유스케이스 다이어그램 | 패키지 다이어그램 | 
|-------------|-----------|
| ![usecase_diagram.svg](/assets/gitbook/post_images/cs/usecase_diagram.svg)            | ![package_diagram.png](/assets/gitbook/post_images/cs/package_diagram.png)          |
...
| 유스케이스 다이어그램                             | 요구사항들 가운데서 기능적인 요구사항을 유스케이스랑 단위로 표현하고 액터와 시스템의 관계를 표현한 다이어그램                              |
| 패키지 다이어그램 🔴                            | 폴더 모양의 패키지와 점선으로 표시된 의존관계, <<Import>>라는 스테레오 타입 표기를 통해 패키지 다이어그램인지를 파악할 수 있음              |
| UML(Unified Modeling Language)          | 객체지향 소프트웨어 개발과정에서 산출물을 명세화,시각화, 문서화할 시 사용되는 모델링기술과 방법론을 통합해 만든 표준화된 범용 모델링 언어             |
...
2. 인덱스  
검색연산의 최적화를 위해 데이터베이스 내 열에 대한 정보를 구성한 데이터 구조
   + 적용기준: 분포도 10~15%
   + 인덱스가 걸려있는 컬럼은 ```HAVING```절 사용시 인덱스 미사용
   + ```CREEATE [UNIQUE] INDEX 인덱스명 ON 테이블명(컬럼명1, 컬럼명2, ...)``` 🔴
3. 뷰
...
+ 스키마 🔴
  + 데이터베이스 구조, 제약조건 등의 종보를 담고 있는 기본적인 구조
  + 외부/개념/내부 3계층으로 구성
+ 테이블
  + 데이터 저장 공간
+ 뷰
...
| JSON 🔴                                 | 비동기 브라우저/서버 통신을 위해 '속성-값 쌍', '키-값 쌍'으로 이루어진 데이터 오브젝트를 전달하기 위해 인간이 잃을 수 있는 텍스트를 사용하는 개방형 표준 포맷<br> AJAX에서 많이 사용되고 XML을 대체하는 주요 데이터 포맷 |
...
+ 쿼츠 스케줄러
  + 스프링 프레임워크에 플러그린되어 수행하는 작업과 실행 스케줄을 정의하는 트리거를 분리하여 유연성을 제공하는 오픈 소스 기반 스케줄러
...
+ 회귀 테스트
  + 회귀 테스트는 오류를 제거하거나 수정한 시스템에서 오류 제거와 수정 때문에 새로이 유입된 오류가 없는지 확인하는 일종의 반복 테스트
...
| 회귀 테스트                             | 변경 또는 수정된 코드에 대하여 새로운 결함 발견 여부를 평가하는 테스트로 유지보수 시 많이 사용되는 테스트                       |
...
| 용어                                 | 설명                                                                                       |
|------------------------------------|------------------------------------------------------------------------------------------|
| DES(Data Encryption Standard)      | 56bit의 키를 이용, 64bit의 암호문 블록으로 만드는 블록 암호 방식의 미국표준 암호화 알고리즘                                |
| AES(Advanced Encryption Standard)  | 고급 암호화 표준이라고 불리는 AES 알고리즘은 DES를 대체한 암호 알고리즘                                              |
| SEED                               | 안정성, 신뢰성이 우수한 128bit 단위의 대칭 키 암호화 알고리즘                                                   |
| 디피-헬만                              | 암호 키를 교환하는 방법으로서 두 사람이 암호화되지 않은 통신망을 통해 공통의 비밀 키를 공유할 수 있도록 하는 방식                        |
| RSA(Rivest Shamir Adleman)         | 비대칭 키 암호 방식 중에서 가장 널리 쓰이는 방식으로 소인수 분해의 어려움을 이용한 방식                                       |
| SHA(Secure Hash Algorithm)         | 미국 국가안보국이 설계한 미국 국가 표준 지정된 해시 암호화 알고리즘, SHA-1, SHA-2                                     |
| MD5(Message-Digest Algorithm 5) 🔴 | MD4를 대체하기 위해 사용하는 128bit 해시 암호화 알고리즘                                                     |
| HAS-160                          | 국내 표준 서명 알고리즘을 위해 개발된 알고리즘으로 MD5와 SHA-1의 장점을 취하여 개발된 해시 알고리즘                             |
...
print(7/3);    // 2.33333 파이썬에서만 '/' 기호값으로 소수점까지 출력
               // '%' 연산은 다른 언어들과 동일
```
```
...
![process-status.png](/assets/gitbook/post_images/cs/process-status.png)
...
제어 정보들은 모두 운영체제가 제공하는 프로토몰에 의해 송신 측에서는 계층을 지날 때마다 덧붙여서 추가되고, 수신 측에서는 계층을 지날 때마다 제거  
